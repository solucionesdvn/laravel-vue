...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: `Salida #${props.productExit.id}`, href: null },
];
...
<template>
  <Head :title="`Salida #${productExit.id}`" />
        ...
        <CardHeader>
          <div class="flex justify-between items-center">
            <CardTitle class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white">
              Detalles de la Salida #{{ productExit.id }}
            </CardTitle>
            <Button as-child variant="outline">
                <Link :href="route('product-exits.index')">Volver al Listado</Link>
            </Button>
          </div>
        </CardHeader>
        <CardContent class="space-y-6 pt-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p class="text-gray-800 dark:text-gray-200">{{ formatDate(productExit.date) }}</p>
            </div>
            <div>
              <p class="text-gray-500 dark:text-gray-400 font-semibold">Motivo:</p>
              <p class="text-gray-800 dark:text-gray-200">{{ productExit.reason }}</p>
            </div>
            <div>
              <p class="text-gray-500 dark:text-gray-400 font-semibold">Registrado por:</p>
              <p class="text-gray-800 dark:text-gray-200">{{ productExit.user.name }}</p>
            </div>
            <div class="md:col-span-3">
              <p class="text-gray-500 dark:text-gray-400 font-semibold">Notas:</p>
              <p class="text-gray-800 dark:text-gray-200">{{ productExit.notes || 'Sin notas.' }}</p>
            </div>
          </div>

          <!-- Tabla de Items -->
          <div class="border-t dark:border-gray-700 pt-4">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-4">Productos Retirados</h3>
            <div class="overflow-hidden border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                <Table>
                    ...
                    <TableBody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                        <TableRow v-for="item in productExit.items" :key="item.id">
          ...
          <div class="flex justify-end pt-4 border-t dark:border-gray-700">
            <div class="text-xl font-bold text-gray-800 dark:text-white">
              Valor Total de Salida: {{ formatCurrency(productExit.total) }}
            </div>
          </div>
